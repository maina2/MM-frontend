  ...
  if (isLoading) {
    return (
      <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "60vh" }}>
        ...
        <Typography variant="h6" sx={{ mt: 3, fontWeight: 500 }}>Loading your order...</Typography>
      </Box>
    );
  }
  ...
  if (error || !order) {
    return (
      <Container maxWidth="md" sx={{ py: 4 }}>
        <Paper elevation={0} sx={{ p: 4, borderRadius: 2, bgcolor: theme.palette.error.light, color: theme.palette.error.contrastText }}>
          <Typography variant="h5" gutterBottom fontWeight="bold">Order Not Found</Typography>
          <Typography variant="body1" paragraph>{error ? "We couldn't load your order information." : "The requested order could not be found."}</Typography>
          <Button variant="contained" onClick={() => navigate("/orders")}>Go to Orders</Button>
        </Paper>
      </Container>
    );
  }

  const steps = ["Payment Initiated", "Payment Confirmed", "Processing", "Completed"];
    ...
    <Container maxWidth="lg" sx={{ py: { xs: 3, md: 4 } }}>
      ...
      <Box sx={{ mb: 3, textAlign: { xs: "center", md: "left" } }}>
        ...
        <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, alignItems: { xs: "center", sm: "center" }, gap: 1 }}>
          <Typography variant="subtitle1" color="text.secondary">Order #{order.id}</Typography>
      ...
      <Card elevation={2} sx={{ borderRadius: 2, mb: 3 }}>
        <CardContent sx={{ px: { xs: 2, md: 3 }, py: 3 }}>
          <Typography variant="h6" fontWeight="medium" gutterBottom>Order Status</Typography>
          <Stepper
          ...
          {paymentStatus === "pending" && (
            <Alert severity="info" variant="outlined" sx={{ mt: 2, borderRadius: 1.5 }}>
              Please complete the payment on your phone. We're waiting for confirmation...
            </Alert>
          )}
          {paymentStatus === "paid" && (
            <Alert severity="success" variant="outlined" icon={<CheckCircleIcon />} sx={{ mt: 2, borderRadius: 1.5 }}>
          ...
          <Card elevation={2} sx={{ borderRadius: 2 }}>
            <CardContent sx={{ px: { xs: 2, md: 3 }, py: 3 }}>
              ...
              {isMobile ? (
                ...
                <Accordion defaultExpanded={order.items.length < 5} disableGutters elevation={0}>
                  <AccordionSummary 
                  ...
                  >
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', pr: 2 }}>
                      <Typography fontWeight="medium">
                        <Badge badgeContent={order.items.length} color="primary" sx={{ mr: 1 }}>
                    ...
                    {order.items.map((item, index) => (
                      <React.Fragment key={item.id}>
                        <Box sx={{ display: "flex", justifyContent: "space-between", py: 1.5 }}>
                          <Box>
                            <Typography variant="body1" fontWeight="medium">{item.product.name}</Typography>
                            <Typography variant="body2" color="text.secondary">Qty: {item.quantity}</Typography>
                          </Box>
                          <Typography variant="body1">KSh {(item.price * item.quantity).toLocaleString()}</Typography>
                        </Box>
              ...
              <Box sx={{ display: "flex", justifyContent: "space-between", py: 1 }}>
                ...
                <Typography variant="body1">KSh {subtotal.toLocaleString()}</Typography>
              </Box>
              <Box sx={{ display: "flex", justifyContent: "space-between", py: 1 }}>
                <Typography variant="body1">Shipping</Typography>
                <Typography variant="body1">KSh {shipping.toLocaleString()}</Typography>
              </Box>
              <Divider sx={{ my: 2 }} />
              <Box sx={{ display: "flex", justifyContent: "space-between", py: 1 }}>
        ...
        <Grid item xs={12} md={4}>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
            ...
            <Card elevation={2} sx={{ borderRadius: 2 }}>
              <CardContent sx={{ px: { xs: 2, md: 3 }, py: 3 }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 2 }}>
                  ...
                  <Typography variant="h6" fontWeight="medium">Shipping Info</Typography>
                </Box>
                <Typography variant="body2" color="text.secondary">Delivery Address</Typography>
                <Typography variant="body1" gutterBottom>{order.shipping_address || "Default Address"}</Typography>
            ...
            <Button
              ...
              sx={{ py: 1.5, borderRadius: 2, fontWeight: "medium", textTransform: "none" }}
            >
              View All Orders
            </Button>
          </Box>
